...
6. 授权码必须保存 HMAC 摘要用于验证，并以 `ACCESS_CODE_ENCRYPTION_KEY` 加密保存可恢复副本；完整码只能向 staff 管理页面展示，不得写入日志、审计详情或公开页面。
...
12. 聊天临时身份只在HttpOnly Cookie保存随机明文，数据库只保存摘要；复用参与者时必须同时校验聊天室、IP和设备特征。
13. 已删除的视频授权码必须释放码值并允许重新生成；复用时创建新授权记录，不得改写旧记录或其观看历史。未删除视频码和现有聊天室码仍须保持全局唯一。
14. 聊天图片必须校验访客会话、CSRF、大小、真实图片内容和像素上限，统一安全转码后保存到 `CHAT_IMAGE_DIR`；不得使用用户文件名组成路径或创建公开图片目录。
...
- 授权码不得直接明文入库或写入日志，只有加密密文可持久化；访客会话令牌与临时身份明文不得入库；普通聊天页面只显示脱敏IP。
