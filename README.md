...
- 授权码保存不可逆HMAC摘要与加密密文；完整码仅向staff管理端开放
- 删除授权码或停止共享时立即撤销相关活动会话；已删除码值可重新生成使用
...
- 内置常用表情选择器，支持发送JPG、PNG、GIF、WebP图片并实时广播
- 图片经过真实内容、大小与像素校验后统一转为WebP，保存在非公开目录并通过聊天室会话授权读取
- 每位参与者获得聊天室内唯一的小说人物昵称、随机头像和脱敏IP标识
- 同一浏览器使用HttpOnly临时身份标识重复进入同一聊天室时复用昵称与头像
- 内置22组中文小说人物备用昵称
